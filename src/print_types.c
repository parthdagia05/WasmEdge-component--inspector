...
const char *val_type_name(WasmEdge_ValType vt) {
    // ValType is opaque, the API only lets you ask what it is
...
// fixed cap keeps printing allocation-free, wider signatures get "... N more"
#define MAX_TYPES 16
...
void print_function_type(const WasmEdge_FunctionTypeContext *ft) {
    ...
    // the Get calls return the full length, not how many entries they
    // wrote into the buffer, so clamp before printing
